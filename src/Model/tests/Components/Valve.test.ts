...
describe('Valve init', () => {
  test('Valve without volume= max', () => {
    expect(valve.Volume).toBe(Number.MAX_SAFE_INTEGER)
  })
  test('Valve with volume', () => {
    const testValve = new Valve(name, source, testVolume)
    expect(testValve.Volume).toBe(testVolume)
  })
